...
| `TELEGRAM_FORWARD_CHAT_ID`                                             | ID чата для сообщений бота и отзывов о приложении | не задан — пересылка отключена                     |
...
Планировщик `schedule-updater` синхронизирует расписание каждый час с 05:00 до
18:00 по часовому поясу `Asia/Yekaterinburg`. В 19:00 текущий проход и повторы
запросов останавливаются без сохранения неполного расписания. Ручной запуск
`bun run update-schedule` также доступен только с 05:00 до 19:00.
Бот проверяет очередь уведомлений
...
Отзывы отправляются ботом в `TELEGRAM_FORWARD_CHAT_ID` через 5 минут после последнего сохранения (проверка каждые 10 секунд). Правки заново запускают пятиминутное ожидание. Очередь хранится в базе, переживает перезапуск; ошибки отправки повторяются автоматически. Перед запуском обновлённых web и bot примените миграции: `bun run db:migrate`. Ранее сохранённые отзывы автоматически не пересылаются.
